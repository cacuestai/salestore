...
class DevolucionCompra implements Persistible {
    ...
    /**
     * Devuelve una cadena JSON que contiene el resultado de seleccionar la información básica de devoluciones de compras
    ...
    public function insertar($param) {
        ...
        if ($instruccion) {
            ...
            if ($instruccion->execute([':datos_devolucion' => $datosDevolucion])) {
                ...
                $info['id_devolucion'] = $fila['insertar_devolucion_compra'] + 1; // agregar el nuevo ID a la info que se envía al front-end
                echo json_encode($info);
            } else {
                echo $conexion->errorInfo($instruccion);
            }
        } else {
            echo json_encode(['ok' => FALSE, 'mensaje' => 'Falló en el registro de la nueva devolución de compra']);
        }
    }
